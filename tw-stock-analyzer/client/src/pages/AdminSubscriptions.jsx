      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500">尚無訂閱</div>
          ) : (
            <>
                      ...
                      <tr key={sub.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(sub.status)}`}>
                            {sub.status === 'active' ? '啟用' : 
                             sub.status === 'cancelled' ? '已取消' : '已過期'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                          {sub.startedAt ? new Date(sub.startedAt).toLocaleDateString('zh-TW') : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                          {sub.expiredAt ? new Date(sub.expiredAt).toLocaleDateString('zh-TW') : '-'}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
